...
// 카카오 SDK를 서버에서 직접 가져와서 캐시
let sdkCache = null;

function fetchKakaoSDK() {
  return new Promise((resolve, reject) => {
    const url = 'https://dapi.kakao.com/v2/maps/sdk.js?appkey=fc1757fbfd613a6f11f04a76f04d0762&autoload=false&libraries=clusterer';
    https.get(url, (res) => {
      let data = '';
...
${sdk}
  ...
  <script>
    ...
    kakao.maps.load(function() {
      ...
      window.addEventListener('message', function(e) {
        ...
        if (e.data.type === 'FILTER') {
          renderMarkers(e.data.category === '전체' ? trails : trails.filter(t => t.category === e.data.category));
        }
        if (e.data.type === 'SELECT') {
          const t = trails.find(t => t.id === e.data.id);
    ...
    function renderMarkers(list) {
      ...
      list.filter(t => t.gpx).forEach(function(trail) {
        const color = CAT_COLORS[trail.category] || '#10b981';
        const emoji = CAT_EMOJI[trail.category] || '🗺';
        const pos = new kakao.maps.LatLng(trail.gpx.lat, trail.gpx.lng);
        ...
        const marker = new kakao.maps.Marker({ position: pos, image: img, map: map });
...
// 서버 시작 시 SDK 미리 가져오기
console.log('카카오 SDK 가져오는 중...');
fetchKakaoSDK().then(sdk => {
  sdkCache = sdk;
  console.log('SDK 로드 완료! 서버 시작: http://localhost:3001');
